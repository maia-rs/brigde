...
class VagaService:
    ...
    @staticmethod
    def get_vagas_by_modalidade(modalidade):
        """ Busca todas as vagas por modalidade. """
        vagas = db.session.query(Vaga).filter_by(modalidade=modalidade).all()
        return vagas
    
    @staticmethod
    def get_vagas_by_status(status):
        """ Busca todas as vagas por status. """
        vagas = db.session.query(Vaga).filter_by(status=status).all()
        return vagas
